...
# Set up logging
logging.basicConfig(filename='Camera_Thermique.log',filemode='a',format='%(asctime)s %(levelname)-8s [%(filename)s:%(name)s:%(lineno)d] %(message)s',level=logging.WARNING,datefmt='%d-%b-%y %H:%M:%S')
...
class pithermalcam:
    ...
    def __init__(self,use_f:bool = False, filter_image:bool = False, image_width:int=gwidth, image_height:int=gheight, output_folder:str = '/var/www/html/Webcam/'):
        self.use_f=use_f
    ...
    def __del__(self):
        logger.debug("Camera Thermique éteinte.")
    ...
    def _pull_raw_image(self):
        ...
        self._raw_image = np.zeros(24*32) #np.zeros((24*32,))
        try:
    ...
    def _process_raw_image(self):
        ...
        if self._interpolation_index==5:  # Scale via scipy only - slowest but seems higher quality
            ...
            self._image = cv2.applyColorMap(self._image, cmapy.cmap(self._colormap_list[self._colormap_index]))
            self._image = cv2.resize(self._image, (480,320), interpolation=cv2.INTER_CUBIC)
        else:
            self._image = cv2.applyColorMap(self._raw_image, cmapy.cmap(self._colormap_list[self._colormap_index]))
            self._image = cv2.resize(self._image, (480,320), interpolation=self._interpolation_list[self._interpolation_index])
    ...
    def _add_image_text(self):
        ...
        if (self._interpolation_index!=3):
            inter = '- I:'+self._interpolation_list_name[self._interpolation_index]
        else:
            inter = ''
            
        if (self._colormap_index!=0):
            color = '- C:'+self._colormap_list[self._colormap_index]
        else:
            color = ''
        
        if self.filter_image == True: 
            filter = 'Filter:True'
        else: 
            filter = ''
        ...
        if (1/(time.time() - self._t0) > 2):
            fps = '- FPS:{1/(time.time()'
        else: 
            fps = '' 

        text = f'{temp_min:+.1f}{temptxt} > {temp_max:+.1f}{temptxt} {inter} {color} {filter} {fps}'
        ...
        if self._file_saved_notification_start is not None and (time.monotonic()-self._file_saved_notification_start)<1:
            cv2.putText(self._image, 'Image sauvegardee!', (100,150),cv2.FONT_HERSHEY_SIMPLEX, .8, (255, 255, 255), 2)

    def add_customized_text(self,x,y,text):
        """Add custom text to the center of the image, used mostly to notify user that server is off."""
        cv2.putText(self._image, text, (str(x),str(y)),cv2.FONT_HERSHEY_SIMPLEX, .1, (255, 255, 255), 2)
        time.sleep(0.1)
    ...
    def _show_processed_image(self):
        ...
        vidcap.set(3, gheight)  # set Width
        vidcap.set(4, gwidth)  # set Height
        ...
        self._image = cv2.addWeighted(self._image,alpha,image,1-alpha,0)
    ...
    def _print_shortcuts_keys(self):
        ...
        print("X - Change de Colormap - précédant")
        print("C - Change de Colormap - suivant")
        print("F - Affiche le filtre On/Off")
        print("T - Change la température entre C/F")
        print("U - Change Interpolation - précédant")
        print("I - Change Interpolation - suivant")
    ...
    def save_image(self):
        ...
        fname = self.output_folder + "CameraThermique"+ dt.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.jpg'
        cv2.imwrite(fname, self._image)
        self._file_saved_notification_start = time.monotonic()
        print('Capture Image ', fname)
...
if __name__ == "__main__":
    ...
    output_folder = '/var/www/html/Camera_Thermique/'
